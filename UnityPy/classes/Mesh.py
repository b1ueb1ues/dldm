...
class CompressedMesh:
    def __init__(self, reader):
        ...
        if version[0] > 3 or (version[0] == 3 and version[1] >= 5):  # 3.5 and up
            if version[0] < 5:
...
class VertexData:
    ...
    def GetStreams(self):
        streamCount = max([x.stream for x in self.m_Channels]) + 1
        self.m_Streams = {}
        offset = 0
        for s in range(streamCount):
            ...
            self.m_Streams[s] = StreamInfo(channelMask=chnMask, offset=offset, stride=stride, dividerOp=0, frequency=0)
            offset = int(offset + self.m_VertexCount * stride)
            ...
            for i in range(6):
                if channelMask[i]:
                    ...
                    if i in [0, 1]:
                        ...
                        m_Channel.format = 0  # kChannelFormatFloat
                        m_Channel.dimension = 4
                    offset += m_Channel.dimension * MeshHelper.GetChannelFormatSize(m_Channel.format)

    def FixChannel(self):
        if any(
            [x.dimension > 4 for x in self.m_Channels]):  # m_Channels.FirstOrDefault(x => x.dimension > 4) != null: #
            fixStream = max([x.stream for x in self.m_Channels])  # m_Channels.Max(x => x.stream)
            fixChannels = [
                x
                for x in self.m_Channels
                if x.dimension > 0 and x.stream == fixStream
            ]
            stride = 0
            for i, curChannel in enumerate(fixChannels):
                preChannel = fixChannels[i - 1]
                offset = curChannel.offset - preChannel.offset
                preChannel.dimension = offset / MeshHelper.GetChannelFormatSize(preChannel.format)
                stride += offset
            # Fix Last
            ...
            m_Channel.dimension = channelStride / MeshHelper.GetChannelFormatSize(m_Channel.format)
            self.GetStreams()
...
class MeshBlendShape:
    def __init__(self, reader):
        ...
        if version[0] == 4 and version[1] < 3:  # 4.3 down
    ...
    def __init__(self, reader):
        ...
        if version[0] > 4 or (version[0] == 4 and version[1] >= 3):  # 4.3 and up
            ...
            self.m_ShapeVertices = [
                BlendShapeVertex(reader)
                for _ in range(m_ShapeVerticesSize)
            ]
...
class Mesh(NamedObject):
    def __init__(self, reader):
        ...
        if version[0] < 3 or (version[0] == 3 and version[1] < 5):  # 3.5 down
            self.m_Use16BitIndices = reader.read_int() > 0

        if version[0] == 2 and version[1] <= 5:  # 2.5 and down
        ...
        if version[0] > 2 or (version[0] == 2 and version[1] >= 6):  # 2.6.0 and up
            ...
            if ((version[0] > 2017 or (version[0] == 2017 and version[1] >= 4))  # 2017.4
                or ((version[0] == 2017 and version[1] == 3 and version[
                    2] == 1) and self.build_type.IsPatch)  # fixed after 2017.3.1px
                or ((version[0] == 2017 and version[
                    1] == 3) and self.m_MeshCompression == 0)):  # 2017.3.xfx with no compression
            ...
            if self.m_Use16BitIndices:
                self.m_IndexBuffer = [
                    reader.read_u_short()
                    for _ in range(m_IndexBuffer_size // 2)
                ]
                reader.align_stream()
            else:
                self.m_IndexBuffer = reader.read_u_int_array(m_IndexBuffer_size // 4)

        if version[0] < 3 or (version[0] == 3 and version[1] < 5):  # 3.4.2 and earlier
            ...
            if version[0] == 2 and version[1] <= 5:  # 2.5 and down
                ...
                self.m_Tangents = reader.read_float_array(reader.read_int() * 4)  # Vector4
                self.m_Normals = reader.read_float_array(reader.read_int() * 3)  # Vector3
        else:
            ...
            if version[0] == 3 or (version[0] == 4 and version[1] <= 2):  # 4.2 and down
                self.m_BindPose = reader.read_matrix_array()

            self.m_VertexData = VertexData(reader)

        if version[0] > 2 or (version[0] == 2 and version[1] >= 6):  # 2.6.0 and later
            self.m_CompressedMesh = CompressedMesh(reader)

        self.m_LocalAABB = AABB(reader)

        if version[0] < 3 or (version[0] == 3 and version[1] <= 4):  # 3.4.2 and earlier
    ...
    def ProcessData(self):
        if hasattr(self, 'm_StreamData') and self.m_StreamData.path:
            if self.m_VertexData.m_VertexCount > 0:  #
                ...
                self.m_VertexData.m_DataSize = resourceReader.get_data()
        ...
        for chn, m_Channel in enumerate(self.m_VertexData.m_Channels):
            if m_Channel.dimension > 0:
                ...
                if channelMask[chn]:
                    ...
                    componentByteSize = MeshHelper.GetChannelFormatSize(m_Channel.format)
                    componentBytes = bytearray(m_VertexCount * m_Channel.dimension * componentByteSize)
                    for v in range(m_VertexCount):
                        ...
                        for d in range(m_Channel.dimension):
                            ...
                            dstOffset = componentByteSize * (v * m_Channel.dimension + d)
                            componentBytes[dstOffset:dstOffset + componentByteSize] += self.m_VertexData.m_DataSize[
                                                                                       componentOffset:componentOffset + componentByteSize]

                    if self.reader.endian == '>' and componentByteSize > 1:  # if big endian, swap bytes
                        for i in range(0, len(componentBytes), componentByteSize):
                            componentBytes[i:i + componentByteSize] = componentBytes[i:i + componentByteSize][::-1]
                        ...
                        # 2018.2 and up
                            ...
                            for i in range(m_VertexCount):
                                for j in range(m_Channel.dimension):
                                    self.m_Skin[i].weight[j] = componentsFloatArray[i * m_Channel.dimension + j]
                        elif chn == 13:  # kShaderChannelBlendIndices
                            if getattr(self, 'm_Skin', None):
                                self.InitMSkin()
                            for i in range(m_VertexCount):
                                for j in range(m_Channel.dimension):
                                    self.m_Skin[i].boneIndex[j] = componentsIntArray[i * m_Channel.dimension + j]
    ...
    def DecompressCompressedMesh(self):
        ...
        if version[0] < 5:  #
            if m_CompressedMesh.m_BindPoses.m_NumItems > 0:  #
                m_BindPoses_Unpacked = m_CompressedMesh.m_BindPoses.UnpackFloats(16, 4 * 16)
                self.m_BindPose = [
        ...
        # Color
        if hasattr(m_CompressedMesh, 'm_Colors') and m_CompressedMesh.m_Colors.m_NumItems > 0:
    ...
    def InitMSkin(self):
        self.m_Skin = [
            BoneWeights4()
            for _ in range(self.m_VertexCount)
        ]
    ...
    def BytesToFloatArray(inputBytes, size):
        ...
        for i in range(math.ceil(len(inputBytes) / size)):
            ...
            if size == 1:
                ...
                value = float.from_bytes(inputBytes[i * 2:i * 2 + 2], byteorder='big',
                                         signed=True)  # return Half.ToHalf((ushort)BitConverter.ToInt16(value, startIndex));
            elif size == 4:
                value = float.from_bytes(inputBytes[i * 4:i * 4 + 4], byteorder='big', signed=True)
            result[i] = value
        return result
